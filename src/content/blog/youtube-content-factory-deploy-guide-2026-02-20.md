---
title: 'YouTube Content Factory 部署记忆与排坑指南'
description: '基于 HP-G3 的 Ops Hub 集中化部署海外双节点时的全量踩坑与解决方案记录。'
pubDate: '2026-02-21 21:25:00'
tags: ['DevOps', 'Fabric', 'SSH', 'YouTube Factory', 'Deployment']
---
...
* **解法**: 
  核查目标节点（例如 LA 的 `bwg`）的 `/root/.ssh/authorized_keys` 中实际记录的公钥结尾身份。确保 HP-G3 堡垒机里 `/root/.ssh/` 存放有相对应的那把私钥文件，随后精准地在 `inventory.yaml` 里将该节点的 `key_filename` 指向这把正确的私钥。
